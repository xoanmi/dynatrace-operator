...
func (query ConfigMapQuery) GetTenantUUID(connectionInfoConfigMapName string, namespaceName string) (string, error) {
	oneAgentConnectionInfo, err := query.Get(types.NamespacedName{Name: connectionInfoConfigMapName, Namespace: namespaceName})
	if err != nil {
		return "", err
	}
	return ExtractField(&oneAgentConnectionInfo, tenantUUIDName)
}

func (query ConfigMapQuery) GetCommunicationEndpoints(connectionInfoConfigMapName string, namespaceName string) (string, error) {
	oneAgentConnectionInfo, err := query.Get(types.NamespacedName{Name: connectionInfoConfigMapName, Namespace: namespaceName})
	if err != nil {
		return "", err
	}
	return ExtractField(&oneAgentConnectionInfo, communicationEndpointsName)
}
